...
const ServiceSpecs = (props) => {
    ...
    return (
        <div className="service-specs">
            ...
            <div>
                <button className="closing-spec" onClick={props.specClick}>X</button>
            </div>
        ...
        Offering From: {user.first_name} in {user.city}, {user.state}
            ...
            {/* <Link to='/request'> */}
            <button className="request-button" onClick={() => (handleRequestClick(props.service))} type="button" >Request a QPQ from {user.first_name}</button>
            {/* </Link> */}
